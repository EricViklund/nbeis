...
\begin{flalign}
    \label{eq:gDRT}
    Z_{F}\left(\omega\right) &= R_{t} + \sum_{i=1}^{N} \frac{k_{i}}{i \omega - s_{i}} \\
    Z_{gDRT}\left(\omega\right) &= R_{t} + \int_{-\infty}^{\infty} \frac{k\left(s\right) ds}{i\omega - s} \\
    k\left(s\right) &= \nu G\left(\nu\right) \\
    s &= -\nu \\
    Z_{gDRT}\left(\omega\right) &= R_{t} + \int_{-\infty}^{\infty} \frac{G\left(\nu\right) d\nu}{1 + i\frac{\omega}{\nu}}
\end{flalign}
